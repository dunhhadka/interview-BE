...
public final class PhoneUtils {
    ...
    public static boolean isValid(String phone) {
        ...
        return isWeakValid(phone) || isStrictValid(phone);
    }
    ...
    private static boolean isWeakValid(String phone) {
        ...
        return isDigits(phone) && phone.length() >= 7 && phone.length() <= 15;
    }

    private static boolean isStrictValid(String phone) {
        var vnPhoneMatcher = VN_PHONE_PREFIX_PATTERN.matcher(phone);
        if (vnPhoneMatcher.matches()) {
            var prefix = vnPhoneMatcher.group(1);
